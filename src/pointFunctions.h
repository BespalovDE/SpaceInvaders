...
void updatePoints(aliansOptionsStruct &aliansOptions, int &iPoints)
{
    iPoints += priceOfAlien * (aliansOptions.alienStartNumber - aliansOptions.aliensNumber);
...
void GetRecords(recordsStruct &records)
{
    int iRecord = 0;
    int iTempRecord = 0;
    int iMinTempRecord = 0;
    records.recordsNumber = 0;
    std::string line;
    ...
    in.close(); // закрываем файл
    records.maxRecord = iRecord;
    records.minRecord = iMinTempRecord;
    records.iPoints = 0;
}
